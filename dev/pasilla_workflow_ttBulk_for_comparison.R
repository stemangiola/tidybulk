...
tt_test %>%
	...
	tidyHeatmap::heatmap(	.horizontal = sample,		.vertical = symbol,		.abundance = `count`,annotation = c(condition, type),	log_transform = TRUE	)
}

tic()
tt_scaled = plot_densities()
time_df = time_df %>% bind_rows(tibble(step = "plot_densities", time = list(toc()), lines = 7, assignments = 1))

tic()
tt_mds = plot_MDS()
time_df = time_df %>% bind_rows(tibble(step = "plot_MDS", time = list(toc()), lines = 5, assignments = 1))

tic()
plot_adjusted_MDS()
time_df = time_df %>% bind_rows(tibble(step = "plot_adjusted_MDS", time = list(toc()), lines = 12, assignments = 0))

tic()
tt_test = test_abundance()
time_df = time_df %>% bind_rows(tibble(step = "test_abundance", time = list(toc()), lines = 1, assignments = 0))

tic()
plot_MA()
time_df = time_df %>% bind_rows(tibble(step = "plot_MA", time = list(toc()), lines = 8, assignments = 0))

tic()
plot_DE_comparative()
time_df = time_df %>% bind_rows(tibble(step = "plot_DE_comparative", time = list(toc()), lines = 7, assignments = 0))

tic()
plot_heatmap()
time_df = time_df %>% bind_rows(tibble(step = "plot_heatmap", time = list(toc()), lines = 3, assignments = 0))



time_df %>% saveRDS("dev/stats_pasilla_ttBulk.rds")
