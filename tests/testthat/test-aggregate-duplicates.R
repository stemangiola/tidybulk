...
test_that("aggregate_duplicates keeps integer type when keep_integer=TRUE", {
  ...
  res_true <- aggregate_duplicates(se, .transcript = entrez, aggregation_function = mean, keep_integer = TRUE)
  expect_true(is.integer(assay(res_true)))
  
  # Test with keep_integer=FALSE
  res_false <- aggregate_duplicates(se, .transcript = entrez, aggregation_function = mean, keep_integer = FALSE)
  ...
  res_sum <- aggregate_duplicates(se, .transcript = entrez, aggregation_function = sum, keep_integer = TRUE)
  expect_true(is.integer(assay(res_sum)))

  # Test with sum aggregation and keep_integer=FALSE
  res_sum_false <- aggregate_duplicates(se, .transcript = entrez, aggregation_function = sum, keep_integer = FALSE)
...
test_that("aggregate_duplicates keep_integer works with airway dataset", {
  ...
  result_true <- aggregate_duplicates(airway, .transcript = gene_name, aggregation_function = mean, keep_integer = TRUE)
  expect_true(is.integer(assay(result_true)))
  
  # Test with keep_integer=FALSE
  result_false <- aggregate_duplicates(airway, .transcript = gene_name, aggregation_function = mean, keep_integer = FALSE)
