...
This workflow uses **edgeR quasi-likelihood** [@robinson2010edger; @chen2016edgeR], **edgeR robust likelihood ratio** [@chen2016edgeR], **DESeq2** [@love2014deseq2], **limma-voom** [@law2014voom], and **limma-voom with sample weights** [@liu2015voomweights] for differential expression analysis.
...
The fit object produced by each underlying method are stored in as attributes of the `se_mini` object. We can use them for example to perform quality control of the fit.

#### For edgeR

```{r differential-expression-edgeR-fit, eval=FALSE}
library(edgeR)
...
# The DGELRT object contains the original DGEList
dge_obj <- attr(se_mini, "internals")$edgeR_quasi_likelihood
# Access the original DGEList from the DGELRT object
# The counts are stored in the DGELRT object
if (!is.null(dge_obj$counts)) {
  plotMeanVar(DGEList(counts = dge_obj$counts))
} else {
  # If counts are not available, skip the plot
  cat("Counts not available for plotMeanVar\n")
}
