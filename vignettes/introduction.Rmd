...
knit: >
    (function(x, ...){
        # Check if we're running under pkgdown by looking at the call stack
        call_stack <- sys.calls()
        is_pkgdown <- any(grepl("pkgdown", sapply(call_stack, function(x) deparse(x)[1]), ignore.case = TRUE))
        
        # Only use custom knitting for README generation, not for pkgdown
        if (is_pkgdown) {
            # Let pkgdown handle the vignette normally
            return(NULL)
        }
        ...
        readme_content <- readLines(file.path(proj_root, "README.md"))
        readme_content <- gsub("/Users/[^/]+/Documents/GitHub/tidybulk/README_files/figure-gfm/", 
                              "README_files/figure-gfm/", readme_content)
        writeLines(readme_content, file.path(proj_root, "README.md"))
    })
---
