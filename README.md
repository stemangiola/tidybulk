...
doi.org/10.1186/gb-2010-11-3-r25). `normalise_counts` takes a tibble,
...
counts.norm.MDS =
  counts.norm %>%
  reduce_dimensions(value_column = `count normalised`, method="MDS" , elements_column = sample, feature_column = transcript, components = 1:6)

counts.norm.MDS %>% select(sample, contains("Dim"), `Cell type`, time ) %>% distinct()
```

    ## # A tibble: 48 x 9
    ##    sample `Dim 1` `Dim 2` `Dim 3` `Dim 4` `Dim 5` `Dim 6` `Cell type` time 
    ##    <chr>    <dbl>   <dbl>   <dbl>   <dbl>   <dbl>   <dbl> <chr>       <chr>
    ##  1 SRR17…    2.31   0.491 -3.01    0.191   0.120   -0.274 b_cell      0 d  
    ##  2 SRR17…    2.29   0.427 -3.03    0.161   0.127   -0.273 b_cell      1 d  
    ##  3 SRR17…    2.25   0.388 -2.92    0.250   0.112   -0.248 b_cell      3 d  
    ##  4 SRR17…    2.29   0.420 -2.98    0.237   0.0875  -0.248 b_cell      7 d  
    ##  5 SRR17…   -1.46  -2.12  -0.163  -0.0233 -1.18    -0.402 dendritic_… 0 d  
    ##  6 SRR17…   -1.38  -2.17  -0.0592  0.101  -1.07    -0.357 dendritic_… 1 d  
    ##  7 SRR17…   -1.42  -2.12  -0.199  -0.0635 -1.36    -0.342 dendritic_… 3 d  
    ##  8 SRR17…   -1.35  -2.18  -0.127   0.0683 -1.16    -0.362 dendritic_… 7 d  
    ##  9 SRR17…   -2.13  -2.05  -0.0695 -0.501   1.03    -0.396 monocyte    0 d  
    ## 10 SRR17…   -1.95  -1.96   0.0121 -0.537   1.04    -0.370 monocyte    1 d  
    ## # … with 38 more rows

On the x and y axes axis we have the reduced dimensions 1 to 6, data is
...
counts.norm.PCA =
  counts.norm %>%
  reduce_dimensions(value_column = `count normalised`, method="PCA" , elements_column = sample, feature_column = transcript, components = 1:6)
```
...
```
    ...
    ## # … with 1,340,150 more rows, and 10 more variables: `count
    ## #   normalised` <dbl>, TMM <dbl>, multiplier <dbl>, `filter out low
    ## #   counts` <lgl>, PC1 <dbl>, PC2 <dbl>, PC3 <dbl>, PC4 <dbl>, PC5 <dbl>,
    ## #   PC6 <dbl>
...
`annotate_differential_transcription` takes a tibble, column names (as
...
`adjust_counts` takes as arguments a tibble, column names (as symbols;
for `sample`, `transcript` and `count`) and a formula representing the
desired linear model where the first covariate is the factor of interest
and the second covariate is the unwanted variation, and returns a tibble
with additional columns for the adjusted counts as `<COUNT COLUMN>
adjusted`. At the moment just an unwanted covariated is allowed at a
time.
...
counts.cibersort =
    ttBulk::counts %>%
    annotate_cell_type(sample, transcript, `count`, action="add")
...
tidy structure (similar to ttBulk::counts).
...
```
    ...
    ## # A tibble: 48 x 7
    ##    sample     `Dim 1` `Dim 2` `Dim 3` `Dim 4` `Dim 5` `Dim 6`
    ##    <chr>        <dbl>   <dbl>   <dbl>   <dbl>   <dbl>   <dbl>
    ##  1 SRR1740034    2.31   0.491 -3.01    0.191   0.120   -0.274
    ##  2 SRR1740035    2.29   0.427 -3.03    0.161   0.127   -0.273
    ##  3 SRR1740036    2.25   0.388 -2.92    0.250   0.112   -0.248
    ##  4 SRR1740037    2.29   0.420 -2.98    0.237   0.0875  -0.248
    ##  5 SRR1740038   -1.46  -2.12  -0.163  -0.0233 -1.18    -0.402
    ##  6 SRR1740039   -1.38  -2.17  -0.0592  0.101  -1.07    -0.357
    ##  7 SRR1740040   -1.42  -2.12  -0.199  -0.0635 -1.36    -0.342
    ##  8 SRR1740041   -1.35  -2.18  -0.127   0.0683 -1.16    -0.362
    ##  9 SRR1740042   -2.13  -2.05  -0.0695 -0.501   1.03    -0.396
    ## 10 SRR1740043   -1.95  -1.96   0.0121 -0.537   1.04    -0.370
    ## # … with 38 more rows
